...
*Previous Conclusion: delete once have checked over new conc*
If RMSE is the criterion of interest, then the linear mixed effects model with address as random intercept and number of bedrooms as random slope is the winner for the full dataset. This is acceptable to intuition: you might think of rent as having some baseline determined by location or quality of a complex, while the contribution of a single additional bedroom to rent price might be much greater for a unit in Cambridge than in Somerville. Interestingly, the nested model performed well according to AIC and variance of error. Recall that the nested model nested square footage, not number of bedrooms, within address. This, too, seems like a variable that would be significantly more or less important in accordance with desirability of the real estate.
...
Please see the full code appendix below.

```{r warning=F, message=F}
# Take a peek of the data set
